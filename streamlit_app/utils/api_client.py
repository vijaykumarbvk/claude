...
Equivalent to the React src/services/api.ts ApiClient class.
"""
import os
...
def _request(method: str, path: str, **kwargs) -> dict:
    ...
    try:
        ...
        return {"success": False, "message": f"Network error: {exc}", "data": None}
    ...
    try:
        ...
        return {"success": False, "message": f"Unexpected response ({resp.status_code})", "data": None}
...
def update_appointment_status(appointment_id: int, status: str) -> dict:
    return _request("PATCH", f"/api/appointments/{appointment_id}/status", params={"status": status})


def cancel_appointment(appointment_id: int, reason: str) -> dict:
    return _request("DELETE", f"/api/appointments/{appointment_id}", params={"reason": reason})
